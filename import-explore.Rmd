...
So, this is not at all tidy and needs a fair bit of work to lick it into shape.```{r load-packages, include=TRUE, warning=FALSE}
...
4. Harmonise rows and columns between sheets.
...
## Pick df for cross check
chk.2010 <- lapply(rdl.list, function(x) {
...
rdl.full <- lapply(rdl.list, na.cols)
...
After a little bit of thought there didn't seem to be an simple, quick to build way to auto-check for cells below the dataset that were meta-data. Removing rows with long character strings or that had all NAs would work but it was easiest to visually inpect each dataframes to get the range of the dataset proper. 

Viewing the tails of the dataframes shows that between 2009-2015 there are 206 rows of data and for 2005-2008 there are 204 rows. We'll identify the 

```{r clean-below-data}
tail(rdl.list.full[[1]], 11)

## Could remove all rows where len(ONS Area Code) > 9 or NA
