...
Viewing the tails of the dataframes shows that between 2009-2015 there are 206 rows of data and for 2005-2008 there are 204 rows. Metadata from each worksheet are found in the first and penultimate column and consist of long strings of text or NAs that extend partially or fully across the row. Most columns don't contain any metadata so the non-data rows at the end of the dataframe can be dropped by filtering out NAs from a column such as 'Region'.

```{r clean-below-data}
## Drop all NA rows in Region column
rdl <- lapply(rdl, function(x) x[!is.na(x$Region),])
```
